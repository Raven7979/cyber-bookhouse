...
只有元数据、没有真实媒体或逐字稿时，不得添加这两个栏目冒充内容；在正文中准确写明访问限制。宣称完整的视频笔记必须真实取得本地媒体和逐字稿，并在“逐字稿与画面证据”中至少放入 1 张本地封面和 3 张不同的关键画面；关键画面必须紧跟它所证明的时间码段。
...
- `content_type: video` 且 `content_status: full_text` 时，`media_status` 必须是 `local`，`transcript_status` 必须是 `official`、`asr_raw` 或 `asr_proofread`，并且逐字稿栏目至少有 4 个不同的本地图像引用。
